...
namespace Part99_CollectionCommonAndThreadSafe.Examples.ConcurrentDictionary
{
    //reference: https://learn.microsoft.com/en-us/dotnet/standard/collections/thread-safe/how-to-add-and-remove-items
    ...
    public class ConcurrentDictionaryExample
    {
        ...
        public static async Task MainConcurrentDictionaryExample()
        {
            CityInfo[] cityData =
            {
            ...
            _ = Cities.AddOrUpdate(
                ...
                (cityName, existingCity) =>
                {
                    ...
                    if (ci != existingCity)
                    {
                        ...
                        Console.WriteLine($"Duplicate city names are not allowed: {ci.Name}.");
                    }
        ...
        static void TryRemoveCity()
        {
            ...
            {
                Console.Write($"Most recent high temperatures for {retrievedValue.Name} are: ");
                var temps = retrievedValue.RecentHighTemperatures; //  32,47,52,64,49,44,56
                Console.WriteLine(string.Join(", ", temps));
            }
            else
            {
